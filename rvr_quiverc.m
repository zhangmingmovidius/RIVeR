 function hh = rvr_quiverc(x,y,u,v,min_colorbar,max_colorbar,cmap)
...
%     u=varargin{3};
%   v=varargin{4};
%   min_colorbar=varargin{5};
%   max_colorbar=varargin{6};
% end
...
if autoscale,
  ...
  min_colorbar=min_colorbar*autoscale;
  max_colorbar=max_colorbar*autoscale;
...
% vr(vr<=min_colorbar)=min_colorbar;
% vr(vr>=max_colorbar)=max_colorbar;
vrn=round((vr-min_colorbar)/(max_colorbar-min_colorbar)*256);
vrn(vrn<=0)=0;
vrn(vrn>=256)=256;
...
%-----------------------------------------------
%Make Contour
% [Cont,h] =contour(varargin{1}, varargin{2}, (varargin{3}.^2+ varargin{4}.^2).^0.5);
% set(h,'ShowText','on','TextStep',get(h,'LevelStep')*2,'LineWidth',1.5)
% text_handle = clabel(Cont,h);
...
%  for i=1:length(X)
%      
%      quiver(X(i),Y(i),U(i),V(i),'color',[0 0 0],'Autoscale','of')
%  end
%   
